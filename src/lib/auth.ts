...
// SQLite stores dates as ISO strings — hydrate them back to Date objects
...
// Minimal custom adapter for SQLite
const SqliteAdapter = {
  ...
  async updateUser(user: { id: string; name?: string | null; image?: string | null; emailVerified?: Date | null }) {
    db.prepare(`
      UPDATE users SET name = ?, image = ?
      WHERE id = ?
    `).run(user.name ?? null, user.image ?? null, user.id);
  ...
  async getSessionAndUser(sessionToken: string) {
    const row = db.prepare(`
      SELECT s.*, u.id as uid, u.name, u.email, u.image
...
export const authOptions: NextAuthOptions = {
  ...
  providers: [
    ...
    // Dev-only: sign in as the seeded local user without OAuth
    ...(process.env.NODE_ENV === "development"
      ? [
          CredentialsProvider({
            ...
            credentials: {},
            async authorize() {
              const row = db.prepare(`SELECT * FROM users WHERE email = 'dev@localhost'`).get() as Record<string, unknown> | undefined;
              return row
